...
export default function BillboardDetail() {
  ...
  if (loading) return (
    <div className="p-4 sm:p-6 space-y-4 max-w-4xl mx-auto">
      <div className="h-56 sm:h-72 bg-gray-100 rounded-2xl animate-pulse" />
      ...
      <div className="p-4 sm:p-6 max-w-4xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
          ...
          <button onClick={() => setEditing(true)}
            className="w-full sm:w-auto flex items-center justify-center gap-1.5 px-3 py-2 border border-gray-200 bg-[#507c88] rounded-lg text-sm text-white hover:bg-[#507c88]/80 transition">
        ...
        {images.length > 0 && (
          <div className="mb-6">
            <div className="w-full h-56 sm:h-72 rounded-2xl overflow-hidden bg-gray-100 mb-2">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {images.map((img, i) => (
                  <button key={i} onClick={() => setActiveImg(i)}
                    className={"w-14 h-10 sm:w-16 sm:h-11 rounded-lg overflow-hidden border-2 transition shrink-0 " + (activeImg === i ? "border-[#507c88]" : "border-transparent opacity-50 hover:opacity-100")}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-2">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">{billboard.billboardtitle}</h1>
            <p className="flex flex-wrap items-center gap-1.5 text-sm text-gray-500 mt-1">
        ...
        <div className="flex items-baseline gap-1.5 mb-6">
          <span className="text-2xl sm:text-3xl font-bold text-[#507c88]">₹{Number(billboard.price ?? 0).toLocaleString()}</span>
          ...
          {materials.length > 0 && (
            <div>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden">
                {materials.map((m, i) => (
                  <div key={i} className={"flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 px-4 py-2.5 text-sm " + (i > 0 ? "border-t border-gray-100" : "")}>
        ...
        {createdAt && <p className="text-xs text-gray-400 mt-6 text-left sm:text-right">Listed on {createdAt}</p>}
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm sm:text-md text-gray-400">Update your billboard details</p>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          ...
          <button onClick={handleSave} disabled={saving}
            className="w-full sm:w-auto flex items-center justify-center gap-1.5 px-4 py-2 bg-[#507c88] text-white rounded-lg text-sm font-medium hover:bg-[#3d6370] transition disabled:opacity-60">
            ...
            {materials.map((m, i) => (
              <div key={i} className="flex flex-col sm:flex-row gap-2 sm:items-center">
                ...
                <input type="number" placeholder="₹/sq ft" value={m.pricePerSqFt}
                  ...
                  className="w-full sm:w-28 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#507c88]/40 focus:border-[#507c88]" />
                <button type="button" onClick={() => setMaterials((prev) => prev.filter((_, idx) => idx !== i))}
                  className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition self-end sm:self-auto">
                  <TrashIcon size={15} />
                </button>
              </div>
            ))}
            ...
            {mountings.map((m, i) => (
              <div key={i} className="flex flex-col sm:flex-row gap-2 sm:items-center">
                ...
                <input type="number" placeholder="Flat charge (₹)" value={m.flatCharge}
                  ...
                  className="w-full sm:w-36 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#507c88]/40 focus:border-[#507c88]" />
...
function BackBtn({ navigate }) {
  return (
    <button onClick={() => navigate("/partner/billboards")}
      className="flex items-center gap-2 text-xs sm:text-sm text-gray-500 hover:text-gray-800 transition">
