...
export default function Tracking() {
  ...
  if (loading)
    return (
      <div className="p-4 sm:p-6 space-y-4">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="h-40 sm:h-48 bg-gray-100 rounded-2xl animate-pulse" />
        ))}
      </div>
    );
  ...
  if (error)
    return (
      <div className="p-4 sm:p-6 text-sm text-red-500 bg-red-50 border border-red-200 rounded-lg">
        {error}
      </div>
    );
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-4 sm:p-6">
            ...
            <div
              ...
              className="border-2 border-dashed border-gray-200 rounded-xl p-4 sm:p-6 text-center cursor-pointer hover:border-green-400 hover:bg-green-50 transition mb-3"
            >
          ...
          {bookings.map((b) =>
            b.partnerItems?.map((item) => {
              ...
              return (
                ...
                >
                  {/* Header */}
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-4 sm:px-5 py-3 bg-gray-50 border-b border-gray-100">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 px-4 sm:px-5 py-4">
                    {item.coverImage ? (
                      <img
                        src={item.coverImage}
                        alt={item.title}
                        className="w-full sm:w-20 h-44 sm:h-14 object-cover rounded-xl border border-gray-100 shrink-0 shadow-sm"
                      />
                    ) : (
                      <div className="w-full sm:w-20 h-44 sm:h-14 bg-gray-100 rounded-xl flex items-center justify-center text-2xl shrink-0">
                        🪧
                      </div>
                    )}
                  ...
                  <div className="px-4 sm:px-5 pb-5">
                    ...
                    {/* Labels */}
                    <div className="flex justify-between gap-3 mb-4 overflow-x-auto whitespace-nowrap pb-1">
                      {LIFECYCLE.map((step, i) => (
                        <span
                          key={step}
                    ...
                    {nextStatus && (
                      <button
                        ...
                        className={`w-full py-3 text-white text-sm font-bold rounded-full transition disabled:opacity-50 flex items-center justify-center gap-2 ${NEXT_BUTTON_STYLE[nextStatus] ?? "bg-[#507c88] hover:bg-[#3d6472]"}`}
                      >
                        {isActing ? (
                          <>
